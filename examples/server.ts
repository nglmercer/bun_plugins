...
class Utils {
  ...
  /**
   * Intenta limpiar prefijos de Socket.io (como "42") y parsear el contenido
   */
  static parseSocketPayload(rawText: string): any {
    // Buscamos el inicio del primer array '[' o el primer objeto '{'
    const jsonStart = rawText.indexOf("[");
    const objectStart = rawText.indexOf("{");

    // Determinamos dónde empieza realmente el JSON
    const startIndex =
      jsonStart !== -1 && (objectStart === -1 || jsonStart < objectStart)
        ? jsonStart
        : objectStart;

    if (startIndex !== -1) {
      try {
        return JSON.parse(rawText.substring(startIndex));
      } catch {
        return rawText; // Si falla el parseo, devolvemos el texto original
      }
    }
    return rawText;
  }
  ...
  static async parseBody(
    ...
    try {
      if (contentType.includes("application/json")) {
        ...
        // --- NUEVA LÓGICA ---
        // Si el texto parece un mensaje de socket (empieza con números y luego [ o {)
        if (/^\d+[\[\{]/.test(rawText)) {
          body = {
            _type: "socket_io_message",
            rawPrefix: rawText.match(/^\d+/)?.[0],
            data: Utils.parseSocketPayload(rawText),
          };
        } else {
          body = rawText;
        }
        ...
        body = {
          ...
          hex:
            Buffer.from(arrayBuffer).subarray(0, 100).toString("hex") +
